...
class AccessRejectView(discord.ui.View):
    ...
    async def access(
        ...
        together_guild_members: bool = True
        if guild:
            together_guild_members = (
                bool(guild.get_member(self.kicker.id))
                and
                bool(guild.get_member(self.customer.id))
            )
        if together_guild_members:
